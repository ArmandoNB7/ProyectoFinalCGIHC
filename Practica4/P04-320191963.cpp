...
int main()
{
	...
	*/
	...
	while (!mainWindow.getShouldClose())
	{
		...
		glm::mat4 modelCabina = model; // ¡CLAVE! Guardamos la matriz de la cabina a salvo para las ruedas
		modelaux = model;
		...
		color = glm::vec3(1.0f, 1.0f, 0.0f); // Amarillo para la cabina
		...
		//la línea de proyección solo se manda una vez a menos que en tiempo de ejecución
		//se programe cambio entre proyección ortogonal y perspectiva
		color = glm::vec3(0.7f, 0.7f, 0.7f); // Gris claro para el primer brazo
		glUniform3fv(uniformColor, 1, glm::value_ptr(color));
		...
		color = glm::vec3(0.3f, 0.3f, 0.3f); // Gris oscuro para la esfera articular
		glUniform3fv(uniformColor, 1, glm::value_ptr(color));
		...
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		color = glm::vec3(0.7f, 0.7f, 0.7f); // Gris claro para el segundo brazo
		glUniform3fv(uniformColor, 1, glm::value_ptr(color));
		meshList[0]->RenderMesh();
		...
		model = glm::scale(model, glm::vec3(1.0f, 5.0f, 1.0f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		color = glm::vec3(0.7f, 0.7f, 0.7f); // Gris claro para el tercer brazo
		glUniform3fv(uniformColor, 1, glm::value_ptr(color));
		meshList[0]->RenderMesh();
		...
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		color = glm::vec3(0.7f, 0.7f, 0.7f); // Gris claro para la canasta
		glUniform3fv(uniformColor, 1, glm::value_ptr(color));
		meshList[0]->RenderMesh();


		//Hasta aqui va bien el ejercicio y ya sigue la practica
		...
		glm::mat4 modelPerro = model; 

		// --- CUERPO (TORSO) ---
		model = glm::scale(modelPerro, glm::vec3(4.5f, 2.2f, 2.5f)); 
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		...
		// --- CABEZA ---
		model = modelPerro;
		model = glm::translate(model, glm::vec3(2.8f, 1.2f, 0.0f)); 
		...
		color = glm::vec3(0.15f, 0.08f, 0.04f); 
		glUniform3fv(uniformColor, 1, glm::value_ptr(color));
		sp.render();
