...
void CrearStickerPyraminx(glm::vec3 a, glm::vec3 b, glm::vec3 c, const glm::vec3& normal, const glm::vec3& color)
{
	if (glm::dot(glm::normalize(glm::cross(b - a, c - a)), normal) < 0.0f)
	{
		std::swap(b, c);
	}

	glm::vec3 centro = (a + b + c) / 3.0f;
	const float escala = 0.84f;
	const float offset = 0.015f;

	a = centro + (a - centro) * escala + normal * offset;
	b = centro + (b - centro) * escala + normal * offset;
	c = centro + (c - centro) * escala + normal * offset;
	...
	pyraminxStickerColors.push_back(color);
}

void CrearCaraPyraminx(const glm::vec3& A, const glm::vec3& B, const glm::vec3& C, const glm::vec3& color)
{
	const int n = 3;
	glm::vec3 normal = glm::normalize(glm::cross(B - A, C - A));
	std::vector<std::vector<glm::vec3>> puntos(n + 1);

	for (int i = 0; i <= n; ++i)
	{
		float t = (float)i / n;
		glm::vec3 izquierda = glm::mix(A, B, t);
		glm::vec3 derecha = glm::mix(A, C, t);

		for (int j = 0; j <= i; ++j)
		{
			float s = (i == 0) ? 0.0f : (float)j / i;
			puntos[i].push_back(glm::mix(izquierda, derecha, s));
		}
	}
	...
	for (int i = 0; i < n; ++i)
	{
		for (int j = 0; j <= i; ++j)
		{
			CrearStickerPyraminx(puntos[i][j], puntos[i + 1][j], puntos[i + 1][j + 1], normal, color);

			if (j < i)
			{
				CrearStickerPyraminx(puntos[i][j], puntos[i + 1][j + 1], puntos[i][j + 1], normal, color);
			}
		}
	}
}
...
void CrearPyraminx()
{
	glm::vec3 v0(0.0f, 1.837f, 0.0f);
	glm::vec3 v1(0.0f, -0.612f, 1.732f);
	glm::vec3 v2(-1.5f, -0.612f, -0.866f);
	glm::vec3 v3(1.5f, -0.612f, -0.866f);
	...
	pyraminxBase->CreateMeshGeometry(baseVertices, baseIndices, baseVertices.size(), baseIndices.size());

	CrearCaraPyraminx(v0, v2, v1, glm::vec3(1.0f, 0.0f, 0.0f));
	CrearCaraPyraminx(v0, v1, v3, glm::vec3(0.62f, 0.12f, 0.94f)); //MORADO
	CrearCaraPyraminx(v0, v3, v2, glm::vec3(0.0f, 0.0f, 1.0f));
	CrearCaraPyraminx(v1, v2, v3, glm::vec3(1.0f, 1.0f, 1.0f)); //bLANCO
}
...
int main()
{
	...
	*/
	...
	while (!mainWindow.getShouldClose())
	{
		...
		model = glm::scale(model, glm::vec3(3.84f, 3.84f, 3.84f));
		...
		//ESTO ES PARA CONTROLAR LAS ROTACIONES BUENO CON LAS TECLAS
		model = glm::rotate(model, glm::radians(mainWindow.getrotax()), glm::vec3(1.0f, 0.0f, 0.0f));
		...
		glUniform3fv(uniformColor, 1, glm::value_ptr(color));
		if (pyraminxBase) pyraminxBase->RenderMeshGeometry();
