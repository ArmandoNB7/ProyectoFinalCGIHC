...
Skybox::Skybox(std::vector<std::string> faceLocations)
{
	...
	for (size_t i = 0; i < 6; i++)
	{
		...
		unsigned char *texData = stbi_load(faceLocations[i].c_str(), &width, &height, &bitDepth, 3); //el tipo unsigned char es para un array de bytes de la imagen, obtener datos de la imagen 
		if (!texData)
		{
