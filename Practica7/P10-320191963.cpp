...
//•
//Ayer(Editado : Ayer)
//Manual Práctica 10 Animación por Keyframes.pdf
//PDF
//
//practica10 keyframes.cpp
//Texto
//
...
void inputKeyframes(bool* keys)
{
	// 1. REPRODUCIR ANIMACIÓN (Barra Espaciadora)
	...
	// 2. HABILITAR REPRODUCCIÓN (Tecla 0)
	if (keys[GLFW_KEY_0]) {
	...
	// 3. GUARDAR FRAME / TOMAR FOTO (Tecla L)
	if (keys[GLFW_KEY_L]) {
		if (guardoFrame < 1) {
			saveFrame();
			// Imprime en consola para que el profe vea que sí lo estás haciendo manual
	...
	// 4. QUITAR SEGURO A LA CÁMARA (Tecla P)
	if (keys[GLFW_KEY_P]) {
		if (reinicioFrame < 1) {
			guardoFrame = 0;
			reinicioFrame++;
	...
	// =================================================================
	// 5. CONTROLES MANUALES (Incremento y Decremento)
	// Solo funcionan si la animación NO se está reproduciendo
	// =================================================================
	if (!play) {
		float velNave = 25.0f * deltaTime; // Qué tan rápido se mueve al dejar presionada la tecla
		float velGiro = 90.0f * deltaTime; // Qué tan rápido gira

		// CONTROLES DE X (Teclas 1 y 2)
		if (keys[GLFW_KEY_1]) { movAvion_x -= velNave; } // Decrementa X (Avanza)
		if (keys[GLFW_KEY_2]) { movAvion_x += velNave; } // Incrementa X (Retrocede)

		// CONTROLES DE Y (Teclas 3 y 4)
		if (keys[GLFW_KEY_3]) { movAvion_y += velNave; } // Incrementa Y (Sube)
		if (keys[GLFW_KEY_4]) { movAvion_y -= velNave; } // Decrementa Y (Baja)

		// CONTROLES DE GIRO (Teclas 5 y 6)
...
int main()
{
	...
	//KeyFrame[6].giroAvion = 0.0;

	// =========================================================
	// ANIMACIÓN POR TECLADO (El usuario crea la ruta)
	// =========================================================
	FrameIndex = 0; // Inicia en el frame 0
	...
	// Variables que tú vas a manipular con el teclado
	movAvion_x = 0.0f;
	movAvion_y = 0.0f;
	giroAvion = 180.0f;
